...
def cus_reviews(soup):
  try:
    review = soup.find(
        "div", class_="a-section a-spacing-none reviews-content a-size-base")
    ...
    for element in review.find_all("div", {'data-hook': "review"}):
      ...
      flag = 0
      # vote
      for item in element.find_all("span", {'data-hook': "helpful-vote-statement"}):
          flag = 1
          try:
            s = item.get_text()
            if s[0] == "O":
...
data = {'name': cus_res1, 'date': cus_res2, 'title': cus_res3,
        'review': cus_res4, 'vote': cus_res5, 'rating': cus_res6}
...
df["review_stopwords"] = df["text_wo_punct"].apply(
    lambda text: remove_stopwords(text.lower()))
